...
optracker:
	add $s1, $t4, $0 #wefound the operation store it in s1
	add $s3, $s2, $0 #the value we have so far is stored in s3
	add $s2, $0, $0 #set s2 to 0 to figure out the next char 
	addi $s0, $s0, 1 #getting that next char
	j loop
	or $0, $0, $0 
...
multi_loop:
	...
	add $s3, $t4, $0
	j loop
	addi $s0, $s0, 1 #moving to the next character 
...
signcheck:
	slt $t4, $v0, $0       # If v0 < 0, t4 = 1
	beq $t4, $0, int_to_string  # If t4 == 0 (positive), convert
	or $0, $0, $0
	j negativenumber
	or $0, $0, $0
